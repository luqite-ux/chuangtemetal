...
import { buildArticleJsonLd, buildBreadcrumbJsonLd, buildTenantPageMetadata, serializeJsonLd } from "@/lib/seo";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return article ? buildTenantPageMetadata(article.title, article.excerpt, `/news/${slug}`, locale, article.featuredImage || "/images/factory/factory-main.png", "article") : {};
}
...
export default async function NewsArticlePage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeJsonLd(buildArticleJsonLd(article, locale)) }} />
